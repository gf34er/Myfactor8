...
export default function App() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#E0F7FA] flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-10 space-y-8 animate-in fade-in zoom-in duration-500">
        <div className="text-center space-y-2">
          <div className="w-20 h-20 bg-[#00897B] rounded-[2rem] flex items-center justify-center mx-auto shadow-lg text-white mb-4"><Droplet size={40} /></div>
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <div className="relative"><Mail className="absolute left-5 top-5 text-slate-300" size={20} /><input name="email" type="email" placeholder="Email" required className="w-full p-5 pl-14 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 focus:ring-[#00897B] outline-none font-bold text-sm" /></div>
          <div className="relative"><Lock className="absolute left-5 top-5 text-slate-300" size={20} /><input name="password" type="password" placeholder="Пароль" required className="w-full p-5 pl-14 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 focus:ring-[#00897B] outline-none font-bold text-sm" /></div>
          <Button type="submit" style={{ backgroundColor: '#00897B' }} className="py-5">{authMode === 'login' ? <><LogIn size={20}/> Войти</> : <><UserPlus size={20}/> Регистрация</>}</Button>
        </form>
        <Button variant="ghost" onClick={() => setAuthMode(authMode === 'login' ? 'register' : 'login')}>{authMode === 'login' ? "Нет аккаунта? Зарегистрироваться" : "Уже есть аккаунт? Войти"}</Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen pb-32 font-sans transition-all duration-500 overflow-x-hidden" style={{ backgroundColor: settings.bgColor, color: '#1e293b' }}>
          ...
          <div className="space-y-6 animate-in fade-in duration-700">
            {remindersWithStatus.filter(r => r.isDue).map(rem => (
              <div key={rem.id} className="bg-red-50 border border-red-100 p-5 rounded-[2rem] flex items-center gap-4 animate-pulse">
                ...
                <div className="flex-1 text-left"><p className={`${sz.xs} font-black text-red-400 uppercase tracking-widest mb-1`}>Пора вводить!</p><p className={`font-black text-red-700 ${sz.sm}`}>{rem.medName}</p></div>
            ...
            <div className={`grid grid-cols-2 ${cs.gap}`}>
              <div onClick={() => setView('inventory')} className="bg-white p-6 rounded-[2.2rem] shadow-sm border border-slate-100 text-center cursor-pointer active:scale-95 transition-all"><Package className="mx-auto mb-2" size={is.base} style={{ color: settings.accentColor }}/><p className={`${sz.lg} font-black`}>{inventory.length}</p><p className={`${sz.xs} font-bold text-slate-400 uppercase tracking-widest`}>Лекарство</p></div>
              <div onClick={() => setView('history')} className="bg-white p-6 rounded-[2.2rem] shadow-sm border border-slate-100 text-center cursor-pointer active:scale-95 transition-all"><History className="mx-auto mb-2" size={is.base} style={{ color: settings.accentColor }}/><p className={`${sz.lg} font-black`}>{history.length}</p><p className={`${sz.xs} font-bold text-slate-400 uppercase tracking-widest`}>Журнал</p></div>
            </div>
          </div>
        )}
              ...
              return (
                <div key={m.id} className={`bg-white ${cs.p} rounded-[2.2rem] border border-slate-100 shadow-sm space-y-4 relative overflow-hidden`}>
                  {deleteConfirmId === m.id && (
                    <div className="absolute inset-0 bg-white/95 flex flex-col items-center justify-center p-6 z-10 animate-in fade-in text-center"><p className={`${sz.sm} font-black text-slate-800 mb-4`}>Удалить препарат?</p><div className="flex gap-4 w-full px-4"><button onClick={() => setDeleteConfirmId(null)} className="flex-1 py-3 font-bold bg-slate-100 rounded-2xl">Нет</button><button onClick={() => { deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'inventory', m.id)); setDeleteConfirmId(null); }} className="flex-1 py-3 font-bold bg-red-500 text-white rounded-2xl">Да</button></div></div>
                  )}
                  <div className="flex justify-between items-start text-left"><div className="flex-1"><h3 className={`font-black ${sz.base} leading-tight text-slate-800`}>{m.name}</h3><div className="flex flex-wrap gap-x-3 gap-y-1 mt-1.5 font-bold text-slate-400 uppercase text-[10px]"><span>Всего: {m.initialQuantity}</span><span>Потрачено: <span style={{ color: settings.accentColor }}>{Math.round(spent*100)/100}</span></span></div>{m.expiryDate && <span className={`${sz.xs} text-red-400 font-black uppercase flex items-center gap-1 mt-1`}><Calendar size={12}/> Годен до: {m.expiryDate}</span>}</div><button onClick={() => setDeleteConfirmId(m.id)} className="text-slate-200 p-1 hover:text-red-400"><Trash2 size={is.sm}/></button></div>
                  <div className="space-y-2"><div className={`flex justify-between ${sz.xs} font-black uppercase text-slate-400`}><span>Остаток: {Math.round(m.quantity*100)/100} ед.</span><span style={{ color: settings.accentColor }}>{percent}%</span></div><div className="h-3 bg-slate-100 rounded-full overflow-hidden"><div className="h-full transition-all duration-1000" style={{ width: `${Math.min(100, percent)}%`, backgroundColor: settings.accentColor }}/></div></div>
                </div>
              );
            })}
          </div>
        ...
        {view === 'history' && (
          <div className="space-y-4 animate-in slide-in-from-right duration-500 pb-10">
            <div className="bg-white p-2 rounded-[2.2rem] border border-slate-100 shadow-sm flex">
              <button onClick={() => setHistoryTab('list')} className={`flex-1 py-4 rounded-[1.8rem] font-black ${sz.sm} transition-all flex items-center justify-center gap-2 ${historyTab === 'list' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400'}`}><History size={18}/> Список</button>
              <button onClick={() => setHistoryTab('stats')} className={`flex-1 py-4 rounded-[1.8rem] font-black ${sz.sm} transition-all flex items-center justify-center gap-2 ${historyTab === 'stats' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400'}`}><BarChart3 size={18}/> Аналитика</button>
            </div>
            {historyTab === 'list' ? (
                ...
                {Object.entries(groupedHistory).sort((a,b) => b[0] - a[0]).map(([year, yearData]) => (
                  <div key={year} className="space-y-3">
                    <div onClick={() => setExpandedYear(expandedYear === year ? null : year)} className="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm flex justify-between items-center cursor-pointer active:scale-[0.98] transition-all">
                      <div className="flex items-center gap-3"><div className="p-2.5 bg-slate-50 rounded-2xl" style={{ color: settings.accentColor }}>{expandedYear === year ? <ChevronDown size={is.base}/> : <ChevronRight size={is.base}/>}</div><div><p className={`font-black ${sz.base} text-slate-800`}>{year} год</p><p className={`${sz.xs} font-bold text-slate-400 uppercase tracking-widest`}>{yearData.total.toLocaleString()} ед. всего</p></div></div>
                      ...
                      <div className="pl-4 space-y-3 animate-in slide-in-from-top duration-300">
                        {Object.entries(yearData.months).map(([month, monthData]) => (
                          <div key={month} className="space-y-2">
                            <div onClick={() => setExpandedMonth(expandedMonth === month ? null : month)} className="bg-white/60 backdrop-blur-sm p-4 rounded-[1.8rem] border border-white/50 shadow-sm flex justify-between items-center cursor-pointer active:scale-[0.98] transition-all"><div className="flex items-center gap-3"><div className="p-2 bg-white rounded-xl shadow-sm" style={{ color: settings.accentColor }}>{expandedMonth === month ? <ChevronDown size={is.sm}/> : <ChevronRight size={is.sm}/>}</div><p className={`font-black ${sz.sm} text-slate-700 capitalize`}>{month}</p></div><p className={`${sz.xs} font-black px-3 py-1 bg-white rounded-full shadow-sm`} style={{ color: settings.accentColor }}>{monthData.total.toLocaleString()} ед.</p></div>
                            {expandedMonth === month && (<div className="pl-4 space-y-2 animate-in zoom-in-95 duration-200">{monthData.entries.map(h => (<div key={h.id} className="bg-white p-4 rounded-[1.5rem] border border-slate-50 flex justify-between items-center shadow-sm"><div className="flex gap-3 items-center text-left"><div className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center shadow-inner" style={{ color: settings.accentColor }}><CheckCircle2 size={is.sm}/></div><div><p className={`font-black ${sz.xs} text-slate-800`}>{h.medName}</p><p className={`${sz.xs} text-slate-400 font-bold uppercase tracking-tight`}>{h.reason}</p></div></div><div className="text-right"><p className={`font-black ${sz.sm}`} style={{ color: settings.accentColor }}>-{h.dose}</p><p className={`${sz.xs} text-slate-300 font-bold`}>{h.timestamp?.toDate ? h.timestamp.toDate().toLocaleDateString() : '...'}</p></div></div>))}</div>)}
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div className="space-y-6 animate-in fade-in duration-500 text-left">
                <div className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm flex items-center justify-between"><div className="flex items-center gap-3"><div className="p-2 bg-slate-50 rounded-xl text-slate-400"><Calendar size={18}/></div><span className={`${sz.sm} font-black text-slate-800`}>Период:</span></div><select value={selectedStatsYear} onChange={(e) => setSelectedStatsYear(e.target.value)} className={`bg-slate-50 border-none rounded-xl px-4 py-2 font-black ${sz.sm} outline-none`}>{Object.keys(groupedHistory).sort((a,b) => b-a).map(y => <option key={y} value={y}>{y} год</option>)}{Object.keys(groupedHistory).length === 0 && <option value={new Date().getFullYear()}>{new Date().getFullYear()} год</option>}</select></div>
                <div className={`p-6 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden`} style={{ backgroundColor: settings.accentColor }}><div className="relative z-10"><p className={`${sz.xs} font-black uppercase opacity-70 mb-1`}>Итоги {selectedStatsYear}</p><div className="flex justify-between items-end"><div><h2 className={`${sz.xl} font-black leading-none`}>{filteredStats.total.toLocaleString()}</h2><p className={`${sz.sm} font-bold opacity-80 uppercase`}>Ед. введено</p></div><div className="text-right"><h3 className={`${sz.lg} font-black leading-none`}>{filteredStats.count}</h3><p className={`${sz.xs} font-bold opacity-80 uppercase`}>Инъекций</p></div></div></div></div>
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4"><h3 className={`font-black ${sz.sm} text-slate-800 flex items-center gap-2`}><TrendingUp size={18} style={{ color: settings.accentColor }}/> Расход по месяцам</h3><div className="flex items-end justify-between h-32 pt-4 px-1 gap-1.5">{filteredStats.chartData.map(([month, val]) => { const max = Math.max(...filteredStats.chartData.map(d => d[1]), 1); const height = (val / max) * 100; return (<div key={month} className="flex-1 flex flex-col items-center gap-2 group"><div className="w-full bg-slate-50 rounded-t-lg relative overflow-hidden h-full"><div className="absolute bottom-0 left-0 right-0 transition-all duration-1000 ease-out rounded-t-md" style={{ height: `${height}%`, backgroundColor: settings.accentColor }} /></div><span className={`text-[9px] font-black text-slate-400 uppercase`}>{month}</span></div>);})}</div></div>
                <div className="space-y-4 pb-6"><h3 className={`font-black ${sz.sm} text-slate-800 px-2`}>Мишени и причины ({selectedStatsYear})</h3>{filteredStats.reasonList.map(([reason, data]) => { const avgInterval = data.dates.length > 1 ? Math.round((data.dates[0] - data.dates[data.dates.length-1]) / (1000*60*60*24) / (data.dates.length-1)) : null; return (<div key={reason} className="bg-white p-6 rounded-[2.2rem] border border-slate-100 shadow-sm flex justify-between items-center"><div className="flex-1"><div className="flex items-center gap-2 mb-1"><div className={`w-2 h-2 rounded-full ${reason.toLowerCase().includes('профил') ? 'bg-blue-400' : 'bg-red-400'}`} /><h4 className={`font-black ${sz.base} text-slate-800`}>{reason}</h4></div><div className="flex gap-4"><p className={`${sz.xs} font-bold text-slate-400 uppercase text-left`}>Случаев: <span className="text-slate-700">{data.count}</span></p>{avgInterval && <p className={`${sz.xs} font-bold text-slate-400 uppercase`}>Раз в {avgInterval} дн.</p>}</div></div><div className="text-right"><p className={`font-black ${sz.sm} text-slate-800`}>{data.total.toLocaleString()}</p><p className={`${sz.xs} font-bold text-slate-300 uppercase`}>ед. всего</p></div></div>);})}</div>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'reminders' && (
          <div className="space-y-5 animate-in slide-in-from-right duration-500 pb-10 text-left">
            ...
            {remindersWithStatus.map(rem => (
              <div key={rem.id} className={`bg-white ${cs.p} rounded-[2.2rem] border-2 ${rem.isDue ? 'border-red-200' : 'border-transparent'} shadow-sm space-y-4`}><div className="flex justify-between items-start"><div className="flex gap-4 items-center"><div className={`p-3 rounded-2xl shadow-sm ${rem.isDue ? 'bg-red-50 text-red-500' : 'bg-slate-50'}`} style={{ color: !rem.isDue ? settings.accentColor : undefined }}><Clock size={is.base} /></div><div><p className={`font-black ${sz.base} text-slate-800`}>{rem.medName}</p><p className={`${sz.xs} font-bold text-slate-400 uppercase`}>Раз в {rem.intervalDays} дн.</p></div></div><button onClick={() => deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'reminders', rem.id))} className="text-slate-200 p-2"><Trash2 size={20}/></button></div><div className="flex flex-col gap-2"><div className={`p-4 rounded-2xl flex justify-between items-center shadow-inner ${rem.isDue ? 'bg-red-500 text-white shadow-lg' : 'bg-slate-50 text-slate-500'}`}><span className={`${sz.xs} font-black uppercase tracking-widest`}>След.: {rem.nextDate.toLocaleDateString()}</span><span className={`${sz.xs} font-black uppercase`}>{rem.isDue ? 'Пора!' : `Через ${rem.diffDays} дн.`}</span></div></div></div>
            ))}
          </div>
        )}
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-2xl border-t border-slate-100 px-6 pb-10 pt-4 flex justify-around items-center z-50 rounded-t-[2.5rem] shadow-[0_-10px_25px_-5px_rgba(0,0,0,0.05)]">
        {[{ id: 'dashboard', icon: Droplet, label: 'Обзор' }, { id: 'inventory', icon: Package, label: 'Лекарство' }, { id: 'history', icon: History, label: 'Журнал' }, { id: 'reminders', icon: Bell, label: 'График' }].map(item => (
      ...
      <Modal isOpen={isSettingsModal} onClose={() => setIsSettingsModal(false)} title="Настройки" titleClass={sz.lg}>
        <div className="space-y-8 pb-6 text-left">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-4">
              {['#00897B', '#1E88E5', '#5E35B1', '#E53935', '#FB8C00', '#000000'].map(color => (<button key={color} onClick={() => updateSettings({ accentColor: color })} className={`w-12 h-12 rounded-2xl border-4 transition-all active:scale-90 ${settings.accentColor === color ? 'border-slate-800 scale-110 shadow-lg' : 'border-white'}`} style={{ backgroundColor: color }} />))}
            </div>
          </div>
      ...
      {/* Модалки данных (Med, Inject, Reminder) - сокращенно для экономии, но с полным функционалом */}
      <Modal isOpen={isMedModal} onClose={() => setIsMedModal(false)} title="Новое лекарство" titleClass={sz.lg}>
        <form onSubmit={async (e) => { e.preventDefault(); const d = new FormData(e.target); await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'inventory'), { name: d.get('name'), quantity: parseFloat(d.get('q')), initialQuantity: parseFloat(d.get('q')), expiryDate: d.get('expiry'), unit: 'ед', createdAt: serverTimestamp() }); setIsMedModal(false); }} className="space-y-5"><input name="name" placeholder="Название" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required /><input name="q" type="number" placeholder="Количество" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required /><div className="space-y-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase ml-2">Срок годности</label><input name="expiry" type="date" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required /></div><Button style={{ backgroundColor: settings.accentColor }} type="submit" className="py-4 text-sm">Добавить</Button></form>
      </Modal>

      <Modal isOpen={isInjectModal} onClose={() => setIsInjectModal(false)} title="Ввести дозу" titleClass={sz.lg}>
        <form onSubmit={e => { e.preventDefault(); const d = new FormData(e.target); handleInject(d.get('id'), parseFloat(d.get('dose')), d.get('reason')); }} className="space-y-5 text-left"><select name="id" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required>{inventory.map(m => <option key={m.id} value={m.id}>{m.name} ({Math.round(m.quantity)} ед.)</option>)}</select><input name="dose" type="number" placeholder="Доза (ед.)" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required /><input list="reasons" name="reason" defaultValue="Профилактика" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" required /><datalist id="reasons"><option value="Профилактика" /><option value="Правое колено" /><option value="Левое колено" /><option value="Правый локоть" /><option value="Левый локоть" /></datalist><Button style={{ backgroundColor: settings.accentColor }} type="submit" className="py-4 text-sm">Подтвердить</Button></form>
      </Modal>

      <Modal isOpen={isReminderModal} onClose={() => setIsReminderModal(false)} title="Новый график" titleClass={sz.lg}>
        <form onSubmit={async (e) => { e.preventDefault(); const d = new FormData(e.target); const medId = d.get('medId'); await addDoc(collection(db, 'artifacts', appId, 'users', user.uid, 'reminders'), { medId: medId || null, medName: medId ? inventory.find(m => m.id === medId)?.name : d.get('customName'), intervalDays: Number(d.get('days')), startDate: new Date().toISOString() }); setIsReminderModal(false); }} className="space-y-6"><select name="medId" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm"><option value="">Общий график</option>{inventory.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select><input name="customName" placeholder="Название" className="w-full p-5 bg-slate-50 rounded-2xl border-none ring-2 ring-slate-100 font-bold text-sm" /><div className="text-center space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Интервал (дней)</label><input name="days" type="number" defaultValue="4" className="w-full p-4 bg-slate-50 rounded-[2rem] font-black text-4xl text-center border-none outline-none" required /></div><Button style={{ backgroundColor: settings.accentColor }} type="submit" className="py-4 text-sm">Создать</Button></form>
      </Modal>
    </div>
  );
}